...
### 역 근처 판정 — 승강장 윤곽·출구 기준 50m (v1.29.0)

- `station_nearby` 판정 기준을 **역 중심점에서 150m → 승강장 윤곽·출구에서 50m** 로 바꿨다. 운영 DB 의 역 좌표는
  역사 쪽에 찍혀 있어 승강장이 통째로 중심에서 떨어진 역이 있다(명학 63~104m, 관악 82~139m). 중심점 반경을
...
- OSM 에 승강장 면이 없는 역(4호선 평촌·범계·인덕원)은 **출구 50m 안이거나 역 중심 150m 안**이면 묻는다(v1.29.2). 응답
  `station_nearby.basis` 가 `platform`(승강장·출구 기준) / `exit`(출구 기준) / `center`(역 중심 기준)를 알려 준다.

### 역에서 출발하는 도보 경로 — 역 안/밖 · 타고 온 방향 (v1.28.0)

- 서비스 지역 밖에서 전철로 와 역에서 도보 경로를 시작하면 경로에 지하철 구간이 없어 하차 안내(`egress`)가
  붙지 않았다. 도보(`walk`) 계획의 출발점이 출구 자료가 있는 역 가까이면(v1.29.0 부터 승강장 윤곽·출구 50m) 응답에
  `station_nearby`(`station`·`distance_m`·`question`·`travel_question`·`choices`)를 싣는다. 경로 자체는 바꾸지 않는다.
- 이용자가 역 안이라고 답하면 클라이언트가 `origin_station: {name, travel}` 로 다시 요청한다. `travel` 은 타고 온
  열차의 진행 방향(`north`=상행·서울 방향, `south`=하행, 생략=모름)이고, 선택지 문구는 `choices[].label`
